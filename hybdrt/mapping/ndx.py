...
from ..utils.array import rel_round, group_values
from hybdrt.mapping.filters import iterative_gaussian_filter, adaptive_gaussian_filter, masked_filter, get_adaptive_sigmas
...
def assemble_ndx(x, psi, psi_dim_names, tau, sort_by=None, group_by=None, psi_precision=8, sort_dim_grids=None,
    ...
    return dim_grid_values, psi_mesh, x_out


def filter_ndx(ndx, num_sort_dims, impute=False, impute_groups=False, iterative=True, adaptive=False,
               mask_nans=True,
               filter_func=None, by_group=False, **filter_kw):
    ...
    if by_group:
        ...
        for group in it:
            ...
            #     if mask_nans:
            #         weights = (~nan_obs_index[group_index]).astype(float)
            #         out[group_index] = masked_filter(ndx[group_index], weights, filter_func=filter_func, **filter_kw)
