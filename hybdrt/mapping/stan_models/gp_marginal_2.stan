// independent cov for each parameter
functions {
...
parameters {
    ...
    real<lower=0> rho_R;  // cov length scale
    real<lower=0> rho_lntau;  // cov length scale
    real<lower=0> rho_beta;  // cov length scale
...
model {
    ...
    array[K] matrix[N, N] L_K_R;
    array[K] matrix[N, N] L_K_lntau;
    array[K] matrix[N, N] L_K_beta;
    array[K] matrix[N, N] K_cov_R;
    array[K] matrix[N, N] K_cov_lntau;
    array[K] matrix[N, N] K_cov_beta;

    for (k in 1:K) {
        K_cov_R[k] = gp_exp_quad_cov(x, alpha[k], rho);
        // diagonal elements
        for (n in 1:N) {
    ...
    for (k in 1:K) {
        R_raw[k] ~ multi_normal_cholesky(mu[k], L_K_R[k]);
        lntau_raw[k] ~ multi_normal_cholesky(mu[k], L_K_lntau[k]);
        beta_raw[k] ~ multi_normal_cholesky(mu[k], L_K_beta[k]);
    }
