...
parameters {
    ...
    vector<lower=0>[N] R_raw_dop_neg;
...
transformed parameters {
    ...
    vector[N] R_dop_neg;
    array[K] vector[N] R_raw_dop_offset;
    ...
    R_dop_neg = raw_to_actual(R_raw_dop_neg, R_mu[k_dop_neg], R_scale[k_dop_neg]);
    R_raw_dop_offset = R_raw;
    R_raw_dop_offset[k_dop_neg] = R_raw_dop_offset[k_dop_neg] + R_raw_dop_neg;
}
model {
    // model surface
    ...
    for (k in 1:K) {
        R_raw_dop_offset[k] ~ multi_normal_cholesky(mu[k], L_K[k]);
    ...
    z_dop_neg ~ normal(R_dop_neg, z_dop_neg * sigma_rel_Rp);
