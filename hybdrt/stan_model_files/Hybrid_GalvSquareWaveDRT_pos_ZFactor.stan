data {
	...
	real<lower=0> log_Z_factor_scale;
}
parameters {
    // Offsets
...
transformed parameters {
	...
	// Z_factor
	real<lower=0> Z_factor = exp(log_Z_factor_raw * log_Z_factor_scale);
	
	// calculated voltage
	vector[N_t] V_hat = A_t * x + V_inst + V_baseline;
	...
	vector[2 * N_f] Z_hat = (A_f * x + R_inf * inf_zv + inductance * izv) * Z_factor;
