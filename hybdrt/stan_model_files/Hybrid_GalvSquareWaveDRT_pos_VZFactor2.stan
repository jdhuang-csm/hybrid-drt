...
transformed parameters {
	...
	real<lower=0> v_alpha = v_alpha_raw * v_alpha_scale;
	vector<lower=0>[N_t] v_sigma_tot = sqrt(
	    square(v_sigma_min) + square(v_sigma_res)
	    + square(v_alpha * (V_hat - V_baseline))
	);
